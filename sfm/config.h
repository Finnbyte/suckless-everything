...
/* colors                      fg,  bg */
static const Cpair cdir    = { 5, 0 }; /* pink */
static const Cpair cfile   = { 2, 0 };  /* green */
...
static const char suckless[]   = "/suckless";

/* software */
static const char *videoplayer[]  = { "mpv", "--fullscreen" };
static const char *imageviewer[]         = { "nsxiv", "-a" };
static const char *pdfviewer[]        = { "mupdf", "-I" };
static const char *office[]  = { "libreoffice" };
static const char *imageditor[]         = { "gimp" };
static const char *hexeditor[]           = { "r2", "-c", "vv" };
...
static Rule rules[] = {
	{videos,    LEN(videos),    videoplayer,         LEN(videoplayer)         },
	{images,    LEN(images),    imageviewer,        LEN(imageviewer)        },
	{pdf,       LEN(pdf),       pdfviewer,       LEN(pdfviewer)       },
	{documents, LEN(documents), office, LEN(office) },
	{arts,      LEN(arts),      imageditor,        LEN(imageditor)        },
	{obj,       LEN(obj),       hexeditor,          LEN(hexeditor)          },
};
